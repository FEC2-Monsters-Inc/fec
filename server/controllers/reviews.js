...
module.exports = {
  getReviews: (req, res) => {
    ...
    model.getReviews(req.query)
      ...
      .catch(err => {
        console.log('err ctrl.getReviews: ', err);
  ...
  getReviewMeta: (req, res) => {
    ...
    model.getReviewMeta(req.query)
      .then(({data}) => res.status(200).send(data))
      .catch(err => {
        console.log('err ctrl.getReviewMeta: ', err);
        res.status(500).send(err);
      });
  },
  ...
  addReviews: (req, res) => {
    ...
    model.addReviews(req.body)
      ...
      .catch(err => {
        console.log('err ctrl.addReviews: ', err);
        res.status(500).send(err);
      });
  },
  ...
  updateUseful: (req, res) => {
    ...
    model.updateUseful(req.params.review_id)
      ...
      .catch(err => {
        console.log('err ctrl.updateUseful: ', err),
  ...
  updateReport: (req, res) => {
    ...
    model.updateReport(req.params.review_id)
      ...
      .catch(err => {
        console.log('err ctrl.updateReport: ', err),
        res.status(204).send(err);
      });
  },
};